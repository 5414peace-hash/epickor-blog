...
export const metadata: Metadata = {
  title: 'EpicKor Card News | Swipeable Korea Guides',
  description:
    'Browse EpicKor card-news carousels for Korea travel, food, shopping, culture, beauty, and lifestyle topics.',
  alternates: {
    canonical: '/card-news',
  },
  openGraph: {
    ...
    <Link href={item.href} className="group block h-full">
      <article className="grid h-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        ...
        <div className="p-4">
          ...
          <span className="mt-4 inline-flex text-sm font-black text-red-700">Swipe carousel -&gt;</span>
        </div>
      </article>
    </Link>
  );
        ...
        <div className="container mx-auto grid gap-8 px-4 py-8 md:py-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center">
          <div>
            ...
            <h1 className="mt-3 max-w-4xl font-serif text-4xl font-black leading-tight md:text-5xl">
              Swipeable Korea guides built from EpicKor's Instagram carousels.
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-gray-200">
              Browse visual guides for Korea travel, food, shopping, culture, beauty, and everyday decisions. Each
              carousel links back to the full EpicKor guide when you want the deeper article.
            </p>
            <div className="mt-6 grid max-w-xl grid-cols-3 gap-3 text-center">
              <div className="rounded-lg border border-white/15 bg-white/10 p-3">
                <p className="text-2xl font-black">{items.length}</p>
                <p className="mt-1 text-[11px] font-bold uppercase text-gray-300">Carousels</p>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/10 p-3">
                <p className="text-2xl font-black">{featured?.totalCards || 0}</p>
                <p className="mt-1 text-[11px] font-bold uppercase text-gray-300">Cards each</p>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/10 p-3">
                <p className="text-2xl font-black">{labels.length}</p>
                <p className="mt-1 text-[11px] font-bold uppercase text-gray-300">Topics</p>
              </div>
            </div>
          </div>
          ...
          {featured && (
            <Link href={featured.href} className="group block">
              ...
              <p className="mt-3 text-sm font-black text-red-200">Latest carousel -&gt;</p>
            </Link>
          )}
        </div>
      </section>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3 border-b border-gray-200 pb-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">Newest first, ready to swipe on mobile.</p>
          </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map((item, index) => (
            <CardNewsPreview key={`${item.folder}-${item.slug}`} item={item} priority={index < 4} />
          ))}
        </div>
      </main>
    </div>
