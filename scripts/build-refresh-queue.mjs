...
const PUSH_WORKED = {
  ...
  '365': '2026-08-26 제목/의도 정렬 (내부링크가 아니라 **제목 레버**를 시험한다 — 071과 다른 레버를 써야 9/23에 둘을 구별할 수 있다). 기준선(60일): 85쿼리 543노출 2클릭 0.37%. `korean cider` 111노출 6.3위 0클릭 / 알코올 클러스터 7쿼리 88노출 4.2~8.4위 0클릭 / `chilsung cider vs sprite` 89노출 5.5위 1클릭(유일) / `chilsung cider` 46노출 6.5위 0클릭 / halal 26노출 7.1~8.2위. 본문은 이미 전부 답하고 있었고 결함은 포장이었다 — 제목이 트리비아("Beat Sprite by 11 Years")를 약속하는데 검색자는 예/아니오를 묻고 있었다. 제목을 `Is Korean Cider Alcoholic? Chilsung Cider vs Sprite`로 바꾸고 알코올·카페인·할랄 Q&A를 6번째에서 2번째로 올렸다. 판정 9/23, **순위로**.',
  '360': '2026-08-26 제목/의도 정렬 + 가격 갱신 (365와 같은 레버). 기준선(60일): 81쿼리 685노출 **0클릭**. 칼피스 비교 클러스터 4쿼리 252노출 5.7~7.8위 0클릭(최대 수요) / 알코올 3쿼리 98노출 **2.8~7.1위** 0클릭 / `what does milkis taste like` 56노출 9.7위 / `milkis` 48노출 2.6위 / 정작 `milkis flavours`+`flavors`는 43노출뿐. **제목이 4번째로 큰 의도를 이름으로 달고 있었다.** `Milkis vs Calpis, and Whether Korea\'s Milk Soda Has Alcohol`로 바꾸고 칼피스 비교를 3번째에서 2번째로 올렸다. 가격은 2023-06 GS25 자료가 박혀 있어 2026-04 세븐일레븐 1+1(₩1,500→₩750)로 갱신, 출처 명시. 결정 지점(가격 섹션)에 아마존 CTA 1개 추가 — 아마존 클릭 병목이 이 작업의 목적이다. 판정 9/23, **순위로**.',
  '344': '2026-08-26 커버리지 보강 (365/360과 다른 레버 — 이 글은 제목이 이미 최대 쿼리에 맞아 있었다). 기준선(60일): 78쿼리 382노출 3클릭. `bacchus drink` 67노출 8.1위 / `bacchus f` 54노출 5.4위 2클릭 / `bacchus d vs f` 33노출 4.4위 0클릭 / **`is bacchus d bad for you` 38노출 10.5~10.8위** ← 본문에 "bad for you"·"side effect"·"safe"가 0회였다. 동아제약 공식 제품 페이지에서 실측: D·F **둘 다 카페인 30mg**(같다), F에만 DL-카르니틴 100mg, 용법 "만 15세 이상 1일 1회 1병"이 두 제품 동일. 안전성 H2 + FAQ 추가, 표에 카페인·카르니틴·용법 행 추가. 판정 9/23, **순위로** — 목표는 10.5위 → 페이지 1.',
  '345': '2026-08-26 커버리지 보강 (344와 같은 레버 — 제목이 이미 최대 쿼리 `yonsei cream bread` 86노출을 정면으로 받고 있다). 기준선(60일): 52쿼리 332노출 2클릭. `yonsei cream bread` 86노출 6.2위 / `yonsei bread` 39노출 7.4위 / `yonsei milk cream bread` 32노출 6.3위 / **칼로리 클러스터 3쿼리 21노출 8.0~8.8위** — 쿼리가 `calories 300 500` `400 kcal` `400 500` 처럼 숫자를 더듬고 있는데 본문에 calorie·kcal이 0회였다. 한국 영양DB 2곳(인아웃·지니어트) 교차검증: 130g 401~402kcal, **포화지방 12g = 1일 기준치의 80%**, 지방이 열량의 56%. 영양표 + 초코·시즌 변형 열량 + FAQ 추가하고 `When to Skip It`에 그 수치를 붙였다. 판정 9/23, **순위로**.',
  '329': '2026-08-26 제목/의도 정렬 + 답 표면화 (365·360과 같은 레버). 기준선(60일): 194쿼리 730노출 3클릭 0.41%. 수요가 압도적으로 **상태 질문**이다 — `bts all members completed military service 2026` 83노출 8.8위 / `...military service status 2026` 59노출 9.1위 / `...completed 2026` 55노출 8.8위 / 32노출 8.3위 등 **같은 질문의 변주로 260노출 이상이 8.3~9.7위에 클릭 0**. 본문은 정답을 갖고 있었지만 **제목이 연재 챕터(`BTS Part 3`)를 약속했고 답은 1,000단어 뒤 산문에 묻혀 있었다.** 제목을 `Have All BTS Members Finished Korean Military Service? 2026 Status`로 바꾸고 **전원 입대·전역일 표를 최상단에 신설**(진 2024-06-12·제이홉 2024-10-17 확인해 추가, 슈가가 먼저 입대하고 늦게 전역한 이유=사회복무요원 21개월 설명), FAQ 2개·출처 1개 추가. **내부 경쟁 확인함**: BTS/병역 글이 5편인데 `329`가 3,298노출 7.6위로 사실상 독점(`044` 126노출 18.6위, `142` 1노출). 판정 9/23, **순위로**.',
  '231': '2026-09-01 제목/의도 정렬 + 결손 보강 (365·360과 같은 제목 레버에 344·345의 커버리지 레버를 겹쳤다 — 이 페이지는 둘 다 결손이었다). 기준선(큐 실측): 40쿼리 4~9위 **300노출 0클릭**, 사이트 순위 11.58, deadShare 0.7%·foreignShare 0.7%(즉 노출이 거의 전부 전환가능한 영어 수요). **GSC API 토큰이 죽어(invalid_grant) 쿼리 단위 기준선을 못 뽑았고, 대신 구글 자동완성으로 형태를 쟀다** — `sky university vs kaist`가 직접 뜨는데 제목이 `Korean University Rankings Beyond SKY 2026 Guide`로 **랭킹 목록**을 약속하고 있었다(SERP가 스니펫으로 끝낼 수 있는 형태). 제목을 `SKY vs KAIST: Choosing a Korean University by Major, Not Rank`로 바꾸고, 자동완성 최대 분기 두 개에 대한 결손을 채웠다 — `best university in korea for {전공}`용 **전공→학교 매핑 표 신설**, `korean university scholarships`/`GKS`용 **장학금 섹션 신설**. 교육부·대교협 2026 대학정보공시 등록금 실측(평균 ₩7,270,300 / 사립 ₩8,231,500 / 국공립 ₩4,250,000)을 넣어 ₩ 결손도 해소. **GKS 마감일은 일부러 2027 수치를 쓰지 않았다** — 검색 상위 기사가 2025-09 게재분이라 마감이 이미 지난 것이었고, 1차 확인이 안 돼 "공고판을 보라"로 처리했다. 판정 9/23, **순위로** — 목표는 11.58위 → 페이지 1.',
