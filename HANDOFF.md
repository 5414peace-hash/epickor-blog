...
## Current Snapshot - 2026-08-24 (쇼츠 판정 · 주제 스터디 290씨앗 · 발행 6편 · 스키마 복구)
...
- **`424` 맥도날드 코리아 발행 (`525fd892`).** Tier 2에서 꺼냈는데 **기록해둔 각도보다 나은 기사가 나왔다** —
  "한국 한정 메뉴"가 아니라 **`한국의 맛` 프로젝트**다. 2021년부터 지역 농산물을 이름째로 사서
  카운티 이름을 제품에 박는다: 4년간 **459.2톤 · 2,400만 개 · 617억 경제가치**.
  **정직한 절반도 넣었다** — 617억 중 **567억이 지역 브랜드 가치**이고 농가 소득은 44.9억이다.
  가격은 메뉴 페이지에 없어서 **한국맥도날드 2026-02 인상 발표**를 썼고 그렇다고 명시했다.
- **큐 `167`은 보류했다 (`pending_needs_onsite_verification`).** 8/24 개장은 맞고 가이드 공백도 맞지만
  **오늘 쓰면 안 되는 상태**다. ① **관람료가 1차 출처에 없다**(네이버 AI 요약이 20,200원이라 하지만
  출처가 개인 블로그 2건) ② **주소가 매체마다 다르고 출구 번호가 없다** — 영문 예약 페이지(NOL World)는
  아예 **잠실**로 적어놨다 ③ **이미지가 공공누리 제4유형**이라 사용 불가.
  **`198` 워터밤 교훈** — 확정 전에 쓰면 얼버무린 문장이 남는다. 근거·해제조건은 큐 note와 FACTS에.
