...
- **[다음 근무시간 처리] 대표님 PC는 평일 09:00~19:00에만 열린다 (2026-08-20 지시).**
  손이 필요한 두 건이 대기 중이다:
  1. **추석 카드뉴스 예약** `2026-08-20_200` — **9/7 이전 슬롯 필요**(예매 9/7~11). Meta 로그인 필요.
     대표님 지시대로 같은 날짜에 겹쳐 올린다.
  2. **`413` 빠다코코낫 봉지 사진** → `public/assets/images/_inbox/`. 도표가 썸네일로 남은 마지막 글.

- **카드뉴스 3편 제작 완료, 전부 미예약.** 재고 0이던 상태에서 9/17 절벽 대비 착수분이다.
  `2026-08-20_200` 추석 예매 (ticket-stub) · `2026-08-21_339` 오징어게임 촬영지 (location-slate) ·
  `2026-08-21_344` 박카스 D vs F (spec-split). **비주얼 시스템 3종을 새로 설계**했고
  의도적으로 서로 다르다 — 앞의 둘은 어두운 풀블리드 사진, 셋째는 밝은 컷아웃이라
  그리드에서 연속으로 놓여도 납작해지지 않는다.
