...
- **[2026-09-03] 다음 리프레시 대상은 `232`·`192`** (게이트 통과분, 각 클릭 1).
  `232`는 오늘 고친 `242`의 부모 글이라 같은 거래 층을 넣기 좋다.
  **`278`(맥심)은 넣지 않는다** — 8/26 연구가 다 있지만 클릭 0이라 게이트 아래다.

- **[2026-09-03] 미예약 카드뉴스 재고가 0이 됐다.** `223`·`194`·`438`·`433`·`435`를 09-20~09-24에
  예약해 인스타 달력이 **09-24까지** 찼다. 다음 카드뉴스를 만들면 첫 빈 날은 **09-25**다.
  릴스 쪽 미예약분은 아래 DOSSIER 3편(대표님 폰 리뷰 대기)이 전부다.
