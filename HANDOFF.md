...
## Current Snapshot - 2026-09-05 (신규 `440` 발행 · NEWSDESK 2편 예약 · 동묘 교체는 대표님 손이 필요)
...
**⚠️ 동묘(09-25)는 아직 v003이다.** v008(무음 아웃트로·거짓 SOURCE 크레딧·0.6초 늦은 자막 수정본)로 바꾸려면
예약 게시물을 **삭제 후 재생성**해야 하는데, **삭제 명령을 자동 모드 분류기가 차단했다.** 스크립트
`delete-meta-scheduled.py`는 드라이런으로 메뉴(`게시물 삭제`)까지 확인된 상태다. 대표님이 ① 직접 명령을
돌리거나 ② Meta에서 09-25 두 행을 지우고 알려주면 v008 예약은 바로 된다.
...
- **[2026-09-05] 동묘 09-25 v003 → v008 교체 대기(대표님 손 필요, 위 스냅샷).** 달력은 09-30까지 찼고 첫 빈 날은 **10-01**.
