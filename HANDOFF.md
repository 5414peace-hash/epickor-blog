...
## Current Snapshot - 2026-08-25 (주제 스터디 · 발행 7편 · 보류 3 · 기각 1)
...
**8/25에 한 것 — 새 씨앗 라운드 1회, 발행 1편, 기각 1건, 보류 1건.**
...
- **신규 3편 발행 — `422` 쓰리쎄븐 · `423` 햇반 · `424` 맥도날드 한국의 맛.** 전부 리뷰어 100/100.
  **집필 중 사실이 두 번 뒤집혔고 둘 다 게이트가 아니라 확인에서 잡혔다** — `C`/`G`는 가격차로
  추론하지 않고 제품 이미지를 열어 확인했고, **햇반 전자레인지는 필름을 점선까지만 뜯는 것**이 정답이었다.
- **`167` K-Culture Station 보류 (`pending_needs_onsite_verification`).** 8/24 개장·가이드 공백은 맞지만
  ① **관람료가 1차 출처에 없고**(네이버 AI 요약의 20,200원은 출처가 개인 블로그 2건) ② **주소가 매체마다
  다르며 출구 번호가 없다**(영문 예약 페이지는 아예 잠실로 적어놨다) ③ **이미지가 공공누리 제4유형**이다.
  근거·해제조건은 큐 note와 FACTS에.
...
- **Tier 2 잔여 3건**: LCK 직관 · 교복 대여 · 라이선스 패션(디스커버리·내셔널지오그래픽·MLB).
  각도는 `output/strategy/topic-study_2026-08-24.md` §10에 명시돼 있다.
