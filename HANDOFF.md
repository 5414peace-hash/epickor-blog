...
- 2026-08-18: **8/31~9/16 예약 완료 — 릴스 7 + 카드뉴스 10, 매일 05:00 KST, Facebook·Instagram 각 1건.**
  ...
  타이틀 카드)을 직접 업로드**했다.
  - **미예약 1건: 카드뉴스 `2026-07-15_musinsa`.** Meta의 예약 상한이 약 30일이라 9/17이 거부됐다
    (9/16까지만 입력됨). **8/19 이후 아무 때나 걸면 된다.**
  - **남은 하자: 예약된 릴스 5편의 캡션에 `**` 리터럴이 있다** — 라면(9/1)·우지(9/6)·바나나킥(9/7)·
    야쿠르트(9/8)·영수증(9/12). 인스타는 마크다운을 렌더하지 않는다. **파일은 전부 수정돼 있고**
    (`output/reels/{slug}/instagram-caption.txt`) 복사해 붙이면 된다. Meta의 예약글 수정 메뉴가
    자동화에 안 걸려서 미완이다 — 상세는 FACTS 2026-08-18.
  - 재사용 규칙: **캡션은 예약 전에 확정한다.** 예약 후 수정은 UI로만 되고 그마저 불안정하다.
    쓰는 즉시 `len(text) <= 2200`과 `'**' not in text`를 검사할 것.
