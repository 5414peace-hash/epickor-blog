...
## Current Snapshot - 2026-08-21 (씨앗 게이트 신설 + 첫 적용 218 + 아마존 링크 전수 교체 + 카드뉴스 3편 예약)
...
- **`npm run seeds:check` 신설 — 발행 4~8주 된 글이 살았는지 죽었는지 판정한다.**
  ...
  - **첫 적용 완료: `218` 다이소 (커밋 `ded0b957`).** 쿨다운도 실측 확인 — `worked`를 적고 재실행하니
    목록에서 빠지고 다음 3편이 올라왔다.
  - **다음 물 줄 글: `200` 추석(클릭 23·CTR 1.05%·8.9위, 행사가 09-24~26이라 지금이 창) ·
    `198` 워터밤(스펙은 찼고 사실 갱신·소셜 후보) · `255` 백화점 푸드홀.**
...
- **`218` 다이소 리프레시 — 게이트가 고른 첫 글. 스펙 결손보다 "사실이 낡은 것"이 더 컸다.**
  38클릭·CTR 4.93%·8.5위로 살아 있었고 ₩·한글이 둘 다 비어 있었다. 실제로 고친 것:
  - **가격 체계를 아예 안 말하고 있었다.** 다이소는 **6종 균일가(₩500~5,000)뿐이고 상한은 2006년 이후
    그대로**다. 영어권 가이드 중 이걸 명시하는 곳이 거의 없는데, **매장에서 가격 비교가 불필요해지는**
    사실이라 독자 행동을 바꾼다. 이 한 줄이 이번 리프레시의 핵심이다.
  - **FAQ가 "일본 기업인가"를 얼버무리고 있었다** ("its own local history and retail context").
    답에는 날짜가 있다 — **2023-12-12, 34.21%, 약 5,000억원.** *얼버무린 문장이 가장 값어치 있는
    자리*라는 2026-08-04 규칙이 또 맞았다.
  - **뷰티 조언이 낡아 있었다 — 결손이 아니라 오답이었다.** "다이소는 도구만"은 2024년 아모레퍼시픽·
    LG생활건강이 다이소 전용 라인을 만들면서 깨졌고, **2025 다이소몰 신상 판매 1위가 시트마스크**다.
    **리프레시는 빠진 것뿐 아니라 틀린 것을 찾는 작업이다.**
  - 실물 디테일로 **명동역점 12층 층별 안내 + 4호선 1~2번 출구**를 넣었다(2F·7F·12F면 끝난다).
    반대 정보는 CTA 옆에 — **택스리펀은 지점 복불복**이고 ₩5,000 상한 탓에 ₩15,000 기준을 채우려면
    최소 3개다.
  - 캡션 4개 전수 재확인(2026-08-12 규칙). 전부 사진과 일치했고, 2개는 **프레임에 실제로 보이는 것**
    (다이소 한글 간판 · 주방 벽의 ₩1,000/₩2,000 택)을 말하도록 다시 썼다.
  - 2,464 → 3,821단어. 리뷰어 100/100, 빌드 통과, `audit:image-context` 전체 재실행으로 리포트 복구.
  - **9/23 리프레시 실험을 오염시키지 않았다 — 확인함.** `218`은 `refresh-queue.json`의 잔여 25편에도,
    `refresh-baseline.md` 대조군에도 없다. **두 트랙은 선정 기준이 다르다** — 큐는 "스펙이 뭐가 빠졌나",
    게이트는 "이 씨앗이 지금 벌고 있나"를 묻는다. 게이트 작업은 큐 중단(아래 2번)과 무관하게 계속 가능하다.
