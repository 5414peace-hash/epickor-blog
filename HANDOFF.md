...
## Current Snapshot - 2026-08-25 (주제 스터디 · 발행 8편 · 보류 3 · 기각 2)
...
**8/25에 한 것 — 씨앗 라운드 2회, 발행 2편, 기각 2건, 보류 1건.**

- **`426` 한국 치약 발행** — **축은 리콜이 아니라 불소 규제였다.** 고시 두 개가 어긋나 있다:
  허가 규정은 **1,500ppm**까지 허용하는데 **표준제조기준이 1,000ppm 이하**로 묶고, 맞추면 **신고**,
  넘으면 **허가**다. 그래서 2018년 국내 **559개 중 1,000ppm 초과가 0개**였고 **고불소가 프리미엄
  카테고리**가 됐다. **1차 출처를 직접 읽었다** — law.go.kr 별표 PDF의 `<표 1>`, nedrug의 신고/허가 등록,
  애경 공식 회수 FAQ. 리콜은 뉴스가 점유했지만 **`does korean toothpaste have fluoride`는 소비자용
  답이 0건**이었다. 상세는 FACTS.
- **보조배터리 기각** — 규제 핸들(`airline`·`limit`·`rules`)로 나온 첫 사례라 각도는 진짜였는데
  **Korea.net·VisitKorea 영문 공식 + HaniSeoul 전용 가이드**가 현행 2개 제한까지 반영했다. LCK와 같은 벽.
- **`425` 한국 수건 발행** — **자체 제목 0칸 스캔이 또 맞았다.** 68개 카테고리 중 35칸이 비어 있었고
  수건이 통과했다. **축은 단위 번역이다**: 한국은 `수`(실 굵기)와 **수건 한 장의 그램**으로 팔고
  아마존은 같은 물건을 **GSM**으로 판다. 나눗셈 한 번이면 맞춰진다(40×80cm 180g = **563 GSM**,
  송월 호텔 라인 **500~611 GSM**). **수출본이 더 가볍다** — 아마존 470 GSM 대 국내 스퀘어50 500 GSM.
  가격·답례품(결혼·돌·칠순)·주문제직까지 실측. 상세는 FACTS.
...
- **[대표님 5분] `167` 해제** — 네이버예약에서 **빅뱅 코스모스 본전시** 페이지를 열어 **관람료와
  장소/출구 표기**를 확인해주시면 블로커 ①②가 동시에 풀린다. ③(이미지)은 현장 사진이 가장 깨끗하다.
  8주 창이 열려 있으므로 빠를수록 좋다.
