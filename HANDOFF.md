...
- **게이트 5/7 달성.** 주간 클릭 5월 80 → 8월 274(6일치). 미달은 신규 코호트 CTR(1.08%)과 Trazy —
  단 CTR 게이트는 8/20에 **지표 정의가 틀렸다**고 이미 판정됐고 중앙값으로는 신규가 전 코호트 중 최고다.
- **성장의 출처를 처음 쪼갰다: 블로그 순증 +199 중 신규 발행이 +271, 기존 393편이 −72.**
  **총 클릭 ≈ 발행 편수 × 페이지당 2.3~2.9** — 발행을 멈추면 줄어든다. 저장소에 없던 사실이다.
- **내 첫 결론은 틀렸고 트림 검정이 잡았다** — "옛 글 −16%"가 하락 3편(워터밤·보령머드축제) 빼자 +29.8%.
  같은 유형 오류를 저장소가 잡은 **세 번째** 사례. 7월 코호트의 −45%는 검정을 통과한다.
- **1순위 권고를 실행하다가 그 권고 자체를 축소 정정했다.** "노출의 87%가 5위 밖"은 맞지만
  **실제 공략 가능분은 15.5%**다 — 나머지는 dead-end 4편(162,396노출)·내비게이셔널·비영어·정의형.
...
- **`426` 치약 · `425` 수건 발행.** 치약의 축은 리콜이 아니라 **불소 규제**였다 — 허가 고시는 1,500ppm인데
  **표준제조기준이 1,000ppm**이라 2018년 국내 559개 중 초과가 0개였고 고불소가 프리미엄이 됐다.
  법령 별표 PDF·nedrug 등록부·애경 공식 FAQ를 직접 읽었다. 수건의 축은 **단위 번역**(수·그램 ↔ GSM,
  40×80cm 180g = 563 GSM)이고 **수출본이 국내본보다 가볍다**. 둘 다 상세는 FACTS.
