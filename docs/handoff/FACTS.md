...
- **2026-08-11 — 편의점 앱 가입 정책은 체인마다 다르다. 영어권이 통째로 "한국 전화번호가 필요하다"고 뭉뚱그린 것은 틀렸다.** 대표님이 세 앱의 가입 첫 화면을 캡처해 주셨고, 셋이 서로 다르다:
  ...
  - **⚠️ 아직 확정 아님**: 로그인 화면에 Apple이 보이는 것과 **가입이 끝까지 되는 것**은 다르다. 한국 앱은 소셜로 들여보낸 뒤 다음 단계에서 본인인증을 다시 요구하는 경우가 흔하다(이마트24가 그 구조를 그대로 보여준다). **GS25·CU에서 Apple을 누른 다음 화면**을 확인해야 판정된다. 보관 기능(나만의 냉장고)이 그 계정에서 작동하는지는 또 별개다.
  - **왜 값어치 있나**: 영어권 편의점 가이드 8개가 전부 같은 자리에서 얼버무린다 — *"단기 여행자에겐 실용적이지 않을 수 있다"*. 실제로는 **하나는 막히고 둘은 열려 있는** 서로 다른 정책이고, 그걸 구분해 쓴 영어 페이지가 없다.
  *Verified:* 대표님 제공 앱 스크린샷 3장(이마트24·GS25·CU 가입 화면), 2026-08-11.
