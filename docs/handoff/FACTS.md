...
- **[2분 · 최우선] litt.ly/epickor의 아마존 링크 2개 교체.** 지금 걸린 두 개가 **폐쇄된 계정 태그**를
  물고 있어 눌려도 수익이 0이다. 교체 URL은 FACTS 2026-07-27에 있다 (불닭 오리지널 → `dp/B0C9D5576K`,
  불닭 까르보나라 → `dp/B0C85RZWP3`, 둘 다 `?tag=epickor-20`). **litt.ly는 대표님 계정으로만 수정 가능.**
  인스타 유입이 곧바로 수익 경로에 연결되는 유일한 미연결 구간이다.

- **[완료 2026-08-26] ~~아마존 Associates 대시보드~~ — 확인됨.** 결과는 위 "아마존 대시보드 실측" 참조.
  다음 확인은 **9월 중순**(8/21 링크 수리 효과 판정 가능 시점).

- **[구항목] 아마존 Associates 대시보드 — 5분.** 7/27 이후 **클릭 수와 주문 수**만 확인.
  151일 남은 시한(2027-01-23)에 **3건 중 1건**이고, **8/21 링크 대수리(상위 100편 + 170개 교체)의
  효과를 아무도 안 봤다.** 1건이 29일 걸린 것은 11.43% 전환율 기준 **클릭 ~9회**인데 7월 측정은
  **월 35회**였다 — 리태그 후 클릭이 떨어졌을 가능성이 있고, 그렇다면 시한 안에 3건이 안 나온다.
  (2026-08-25 중간점검에서 "가장 값비싼 5분"으로 판정. 사이트 제휴 CTR 마지막 측정이 7/26이라 한 달 묵었다.)
