...
- **[30초 · 최우선] 인스타 바이오 링크를 `epickor.com/ig`로 바꿔 주십시오.**
  지금의 `epickor.com`도 똑같이 동작하지만, 인앱 브라우저가 referrer를 지워서 **GA4에서 인스타 유입을
  `direct` 3,753세션과 구별할 수 없다.** `/ig`는 배포된 리다이렉트라 바꾸기만 하면 된다.
