...
- **2026-08-18 — Meta Suite 예약글 캡션 수정 경로는 자동화가 어렵다 (미해결).** 목록 행 `...` → `게시물 관리 >` 플라이아웃 안에 `수정`이 있는데, 플라이아웃이 스크립트 실행 간에 닫히고 호버 재현이 불안정하다. **주의: 좌측 내비게이션에도 `수정` 라벨이 x=24에 있어**, 위치로 거르지 않으면 메뉴 커스터마이즈 창이 열린다. 예약 자체는 스크립트로 안정적이므로, **캡션은 예약 전에 확정하는 것이 유일하게 확실한 방법**이다.
  *Verified:* 6회 시도 전부 실패, 2026-08-18.
