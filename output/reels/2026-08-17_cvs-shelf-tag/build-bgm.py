"""
Synthesise the audio bed for the CVS Shelf Tag reel, locked to its event grid.
...
STRUCTURE
  ...
  This reel's grid is identical to the beauty reel's — four products, all with a list
  price — so only the key differs.

  NOTE: third copy of this synth. Extract it if a fourth reel needs a bed.
"""
import array
import math
import wave
...
# ------------------------------------------------------------------ #
# B2 sus2, a tone above the beauty reel's A2. Brighter, for a fluorescent-lit subject.
PARTIALS = [(123.47, 0.34), (185.00, 0.22), (277.18, 0.13), (493.88, 0.035)]
dur = NSAMP / SR
...
for bi in range(NPROD):
    ...
    for local, kind in BEATS:
        # Every product here has a list price, so no block skips the strike tick.
...
path = 'output/reels/2026-08-17_cvs-shelf-tag/bgm-counter-bed.wav'
