#!/usr/bin/env node
/**
 * Reel 379 — rebuild the audio timeline, re-align, and derive the cut plan.
 *
 ...
 *
 *   node output/reels/2026-08-13_379/rebuild-timeline.mjs [gapFrames]
 */
import { execFileSync } from 'node:child_process';
import fs from 'node:fs';

const DIR = 'output/reels/2026-08-13_379';
const FPS = 30;
const LEAD = 12;
...
const CUT_FIRST_BEAT = [0, 3, 7, 10, 13, 17];
/** The spoken CTA beat — the outro plate must be up while it is said. */
const OUTRO_FIRST_BEAT = 19;
...
fs.writeFileSync(`${DIR}/remotion-props-v01.json`, `${JSON.stringify({
  slug: '379', fps: FPS, width: 1080, height: 1920, durationInFrames: total, audioSegments: segments,
}, null, 2)}\n`);

/* 2. re-align (the ElevenLabs alignment itself is cached on disk) ---------- */

execFileSync('node', ['.claude/skills/reels/scripts/align-reel-captions.mjs', '--slug', '379', '--version', 'v02'], {
  stdio: 'inherit',
});
...
fs.writeFileSync(`${DIR}/render-manifest.json`, `${JSON.stringify({
  slug: '379', fps: FPS, width: 1080, height: 1920,
