"""
Sound for the 바나나킥 DOSSIER reel — effects only, no bed.

This is a derivative of the 우지 script and the synthesis below is unchanged from it: the same
struck bells on the same C major pentatonic table, the same stapler for the opening stamp, the
same mastering. That is deliberate. Three reels are going out on this kit inside one week to be
measured against each other, and if they do not sound like one format then the thing being
measured is not the format.

WHAT IS DIFFERENT HERE, AND WHY
  Three exhibit plates instead of two, so three cards land on a PLATE rather than running a
  full five-line type figure. There is no falling tone, because no number in this reel falls —
  1.6% is a standing fact printed on a bag, not a collapse. And the closing card carries an
  evidence photograph, so the stamp waits for the plate and its caption instead of opening on
  the card. Sounding the verdict over a photograph that is still fading in reads as a mistake
...
YEARS = [1978, 1978, 2025, 2026, 2026]
...
# The 우지 script defines a falling glide here for its collapsing market share. Every sound in
# this reel is struck, so there is nothing to inherit and the helper is not carried over.
...
for c in cuts:
    ...
    if kind == 'open':
        ...
        events.append(('plate', start + 3, 0))
        events.append(('type', start + 16, 0))
        events.append(('stamp', start + 26, 0))
        events.append(('type', start + 59, 0))
        events.append(('type', start + 66, 0))
    elif kind == 'outro':
        events.append(('type', start + 12, 0))
        events.append(('outro', start + 26, 0))

# No falling tone: nothing in this reel is a decline.
...
path = 'output/reels/2026-08-18_banana-kick-dossier/bgm-dossier-bed.wav'
