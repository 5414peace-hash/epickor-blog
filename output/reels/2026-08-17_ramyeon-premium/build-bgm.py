"""
Synthesise the audio bed for the Premium Ramyeon reel, locked to its event grid.
...
STRUCTURE
  ...
  Block 0 has no list-price beat, so it skips that tick. The audio mirrors the visual
  asymmetry rather than papering over it.

  NOTE: this duplicates the synth from the kbeauty reel's build-bgm.py. If a third
  reel needs a bed, extract the synth and keep only the event map per reel.
"""
import array
import math
import wave
...
NPROD = 3
# Blocks with no list price, so no strike tick. Block 0 is the ₩1,000 reference.
NO_LIST = {0}
...
# ------------------------------------------------------------------ #
# G2 sus2, a tone below the beauty reel's A2 — warmer, to sit with a cream ground.
PARTIALS = [(98.00, 0.34), (146.83, 0.22), (220.00, 0.13), (392.00, 0.035)]
dur = NSAMP / SR
...
for bi in range(NPROD):
    ...
    for local, kind in BEATS:
        if kind == 'list' and bi in NO_LIST:
            continue                       # the reference block has no price to strike
        events.append(('tick' if kind == 'list' else kind, base + local))

# Decision grid: cut, one tick per card, footer line, domain chip.
d = HOOK + NPROD * BLOCK
events.append(('cut', d))
for local in [8 + 9 * i for i in range(NPROD)] + [48]:
...
path = 'output/reels/2026-08-17_ramyeon-premium/bgm-counter-bed.wav'
