...
- **[2026-08-18 변경] `final/` 하위폴더를 폐지했다. 폴더는 평평하다.** 대표님 지시:
  *"여긴 왜 또 안에 final 폴더가 또 들어있냐. 다른 폴더들과 동일하게 정리해야지."*
  ...
  - **한 폴더에 산출 렌더는 하나. 가장 최근 것이 최종이다 (2026-08-18 대표님 지시).**
    *"가장 최근자거 파일 하나만 남기고 삭제하면 됨. 가장 최근거가 가장 최신이고 최종이라고 이해하면됨."*
    mtime 기준으로 최신 하나만 남기고 나머지 산출 렌더는 지운다.
  - **단, 소스 영상은 렌더가 아니다.** `assets/` · `render-public/`(컴포지션이 `staticFile()`로
    참조하는 프록시) · `candidate-videos/` · `audio/`(TTS)는 지우면 **재렌더가 불가능해진다.**
    "파일 하나만 남기고"는 산출물 얘기지 소재 얘기가 아니다.
  - **후보 렌더는 확정 후 지운다.** 2026-08-18에 전 폴더를 평탄화하며 superseded 렌더 119개(5.64 GB)를 삭제했다 —
    `output/reels`가 10.1 GB에서 4.5 GB가 됐고, 57개 릴스 폴더 전부 산출 mp4가 정확히 하나다. **폴더에 재생 가능한 MP4가 하나만 있어야 한다** — 대표님이 무음
...
- **따라서 제작 순서를 뒤집는다.** 종전 순서(발행글 선정 → `strategy.md` 작성 → 소재 탐색)는 실패한다. Reel 311에서 전략·씬플랜·ONS·스토리보드를 다 만든 뒤에야 Pexels에 쓸 만한 세로 떡볶이 영상이 **0건**(10쿼리·29후보)이라는 걸 발견해 기획 전체가 폐기됐다. 기록: `output/reels/2026-07-21_311/clip-sources.md`.
- **올바른 순서**:
...
- **소재 게이트는 `npm run footage:gate -- --topic {주제} --query "..." --query "..."`로 돌린다 (2026-08-03 신설).** 가로·세로를 함께 검색하고 9:16 크롭 가이드를 얹은 컨택트시트(`output/footage/{주제}/contact.html`)를 만든다. **시트를 눈으로 보기 전에는 어떤 클립도 승인된 게 아니다.** 스크립트는 후보를 좁혀줄 뿐 판정하지 않는다.
- **가로 16:9 영상을 9:16으로 크롭해 쓰는 것은 정당하다 (2026-07-21 전제를 2026-08-03 대표님 지시로 수정).** 종전에는 세로 영상만 찾아서 Reel 311이 "세로 떡볶이 0건"으로 폐기됐는데, **소재가 없었던 게 아니라 세로만 봐서 없었던 것**이다. 같은 주제를 가로까지 열어 다시 재니 한국 명시 후보 50건, 업스케일 불필요 333건이 나왔다.
  - 목표는 `1080x1920`. **4K(3840x2160) → 크롭 1215x2160 → 다운스케일**이므로 업스케일이 0이다. **FHD(1920x1080) → 크롭 607x1080 → 1.78배 업스케일**은 허용한다. **HD(1280x720)는 2.67배라 거부**한다.
  - **크롭 윈도를 천천히 이동시키는 패닝을 허용한다.** 피사체가 실제로 움직이므로 Reels 2.1 하드리젝트 항목인 `excessive still-image zooms`(정지컷 줌)와 다르다. 단 패닝은 해상도를 늘려주지 않는다.
  - **잘랐을 때 피사체가 살아남는지를 반드시 확인한다.** 넓은 풍경이 프레임 전폭을 쓰면 9:16 크롭에서 무너진다. 컨택트시트의 밝은 띠가 그 판정용이다.
- **국가 정합이 해상도보다 우선한다.** 틀린 나라의 4K는 가치가 0이다. 실제로 해상도순 정렬에서 8K 마닐라 게시장이 1위로 올라왔다. `footage-gate.mjs`는 Pexels 슬러그(유일한 설명문 — `tags[]`는 항상 비어 있다)에서 타국 지명을 걸러내고 한국 명시 클립을 상단에 올린다. `unnamed country` 표시는 "한국"이 아니라 **"슬러그가 침묵함"** 이라는 뜻이므로 그 후보는 프레임을 봐야 한다.

### Reels 2.3 — 정보 속도가 판정 기준이다 (2026-08-17 대표님 지시 "판을 좀 흔들어야할듯")

> 아래 2.2는 유효하되, **푸티지 기반 키트에만** 적용된다. 2.3은 그 위에 얹히는 상위 판정 기준이다.

- **"움직임"이 아니라 "이산적 변화"를 센다.** `MOTION.requireAmbientMotion`("항상 뭔가 움직여야 한다")은
  **켄번스 줌으로 충족된다** — 376이 정확히 그렇게 통과했다. 그 릴스는 5초 동안 화면에서 바뀌는 게
  **자막뿐**이었고(6프레임 실측), 40초에 6컷이었다. 카드뉴스는 15초 주의에 디자인된 7장을 준다.
  **즉 릴스가 카드뉴스보다 느렸다** — 조회 중앙값 700 대 901로 3개월 연속 진 이유가 이것이다.
  판정 질문은 "무엇이 움직이나"가 아니라 **"몇 초마다 화면 상태가 바뀌나"** 다. 목표는 **≤1초/이벤트**.
- **컨택트시트는 컷당 1프레임이라 이 결함을 못 잡는다.** 한 컷 안에서 0.3/0.8/1.4/2.2/3.5/5.0초를
  뽑아 **같은 컷의 프레임끼리** 비교해야 "정지컷+줌"이 드러난다. 신규 키트는 이 검사를 통과해야 한다.
- **`remotion/CounterKit.tsx`(COUNTER 키트)는 푸티지 없는 대안 경로다.** 배경영상·줌·나레이션·자막밴드를
  전부 없애고 제조사 제품컷 + 대형 타이포 + 카운트업 숫자로 간다. 정가에 줄이 그어지고 실가격이
  올라가는 비트는 **캐러셀이 물리적으로 못 하는 동작**이라 영상 포맷의 존재 이유가 된다.
  제품에 검증된 정가·실거래가가 둘 다 있을 때만 성립한다. 첫 적용: `output/reels/2026-08-17_kbeauty-picker/`.
- **인코딩 하한선 8 Mbps는 푸티지 키트 기준이다.** 평면 디자인 프레임은 CRF 17에서 1.98,
  CRF 14에서 2.43 Mbps로 **3스톱 품질 변화에 23%만 반응한다** — 인코더가 품질 타깃이고 콘텐츠에
  그 이상의 정보가 없다는 뜻이다. 전체 해상도에서 글자 경계를 확인해 아티팩트가 없으면 통과시킨다.
- **오디오는 재생 대역에서 측정한다. 전대역 미터는 이 질문에 답하지 못한다 (2026-08-18, 대표님이
  두 번 "소리가 안들림"을 보고한 뒤 확립).** DOSSIER 베드는 **-14.9 LUFS**였고 폰 대역
  시뮬레이션에서도 이미 승인된 릴스보다 **컸는데도** 실제로는 안 들렸다. 원인은 레벨이 아니라
  **배치**다 — 무게가 49~245 Hz에 있었고, 그중 49 Hz 기음 하나가 헤드룸의 3분의 1을 먹었다.
  **폰·노트북 스피커는 그 대역을 재생하지 않으므로**, 피크 정규화가 들리는 성분을 전부 끌어내리는
  대가로 안 들리는 성분을 살리고 있었다.
  **들을 수 있는 기기에서 안 들리면 그건 무음이다 — 미터가 뭐라 하든.**
  - 점검 명령: `ffmpeg -i {mp4} -af "highpass=f=400:p=2,highpass=f=400:p=2,lowpass=f=9000:p=2,volumedetect" -f null -`
    **첫 3초를 따로 잰다.** 판단은 거기서 내려진다.
  - 처방: 지속음을 **392~784 Hz**에 두고(비브라토를 걸어야 톤이 아니라 기계로 읽힌다), 클릭에
    **1.6~3.7 kHz 공진 바디**를 준다. 저역은 헤드폰용 몸통만큼만 남긴다. 같은 LUFS에서 **+9.5 dB**를 얻었다.
  - **이건 바닥을 올리는 것과 다르다.** 이벤트에 가청 성분을 얹는 것이라 액센트 분리를 깎지 않는다.
- **Remotion이 낸 렌더는 무음 AAC 트랙을 달고 나온다 — 폴더에서 아무 파일이나 재생하면 소리가 없다
  (2026-08-18 대표님 "소리가 안들림").** 실측: 전 릴스 폴더의 작업본이 전부 `max_volume -91.0 dB`이고
  오디오는 `final/`의 먹싱본에만 있다. 즉 **"소리가 안 난다"는 보고를 받으면 베드부터 의심하지 말고
  대표님이 어떤 파일을 열었는지부터 확인한다.** 이 건에서 최종본은 정상이었고, 폰이 통과시키는
  500 Hz 이상 대역만 재면 오히려 CVS 릴스보다 4.4 dB 컸다.
  **작업본은 이름을 바꾸는 정도로는 부족하다 — 최종 확정 후 지운다.** 폴더에 재생 가능한 영상이 딱 하나만 남아야 한다.
- **`remotion/`은 `tsconfig.json`의 `exclude`에 있다 — `npx tsc -p tsconfig.json`은 릴스 코드를
  한 줄도 검사하지 않는다 (2026-08-18 확인).** 통과 메시지를 타입 검사로 착각하지 말 것. 실제로
  `body: string`을 `string[]`로 바꾼 뒤에도 전체 tsc가 깨끗했고, 파일을 직접 지정해서야 오류
  4건이 나왔다. 릴스 파일은 이렇게 검사한다:
  `npx tsc --noEmit --jsx react-jsx --esModuleInterop --skipLibCheck --module esnext --moduleResolution bundler --target es2020 --strict remotion/{파일}.tsx`
- **`loadFonts()`는 컴포넌트 안에서 호출한다.** 모듈 최상위에 두면 `delayRender` 핸들이 렌더 탭 준비
  전에 생겨 `"bundled webfonts" not cleared after 28000ms`로 죽는다(376 계열 키트는 이미 맞게 되어 있다).
- **BGM: `output/bgm/youtube-audio-library/`의 3곡은 인스타에 쓸 수 없다 (2026-08-17 확인).**
  그 폴더의 `LICENSES.md`가 직접 경고한다 — 채널 로그인 상태에서 **"for YouTube use"** 로 받은
  마스터이고 비유튜브 플랫폼 재사용 전 재확인 필요. 인스타·메타용은 **직접 합성**하거나
  Pixabay·Mixkit(CLAUDE.md 승인 소스, 상업 무료·표기 불요)에서 받는다.
  - **이벤트 기반 키트는 합성이 스톡 곡보다 낫다.** 키트의 프레임 번호로 베드를 만들면 컷마다
    저역 타격, 카피 비트마다 틱, 가격 카운트업에 마이크로틱 런이 붙는다 — **구매한 곡은 절대
    이 그리드에 못 맞는다.** numpy 없이 stdlib(`array`+`math`+`wave`)로 25초/46이벤트가 8.6초에
    나온다. 참조: `output/reels/2026-08-17_kbeauty-picker/build-bgm.py`.
  - **동기는 실측한다.** 컷 프레임마다 0.16초 창의 `max_volume`을 재고 컷 사이 구간과 비교한다.
    실측 기준선: 컷 -3.7~-5.1 dB vs 사이 -7.3~-10.7 dB.
- **오디오 마스터링 3대 함정 (2026-08-17 실측, 잘못된 버전 3개를 거쳐 확립).**
  목표는 인스타 정규화 타깃 **-14 LUFS**, True Peak는 **0 dBTP 미만**.
  - **`alimiter`의 `level`은 기본값이 `true`라서 리미팅 후 다시 풀스케일로 올린다 — 천장이 무효화된다.**
    증상이 반직관적이다: **`limit`을 낮추면 파일이 더 커진다**(-0.1 → +0.3 dBTP). 안전 리미터로 쓸 때는
    **`level=false`가 필수**다.
  - **그 오버슈트 폭은 콘텐츠가 정한다. 1.8 dB는 하한이지 상수가 아니다 (2026-08-18 실측).**
    DOSSIER 베드는 WAV에서 **-5.0 dBFS**였는데 `-c:a aac` 후 **-0.0 dBFS** — **5 dB**가 올랐다.
    원인은 래칫·타이프 클릭이 **어택 0의 생노이즈 버스트**라 나이퀴스트까지 에너지가 있고 AAC의
    MDCT가 거기서 링잉하는 것. **리미터로 5 dB를 되사면 프로그램 레벨이 그만큼 죽는다.**
    올바른 수정은 **소스의 클릭 모양**이다 — 어택 램프 0.8 ms + 약 8 kHz 위 원폴 롤오프를 넣으니
    30fps에서 여전히 "딸깍"으로 들리면서 오버슈트가 **5.0 → 0.1 dB**로 사라졌고, 부수적으로
    라우드니스도 0.6 dB 올랐다. **인코딩 전후를 반드시 각각 측정할 것** — WAV만 보면 못 잡는다.
  - **희소한 베드에서 라우드니스는 "이벤트를 늘려서" 얻는다. 바닥을 올리면 안 된다 (2026-08-18).**
    LUFS는 K-weighting이라 150 Hz 아래를 크게 깎는다. 모터 험만 있는 베드는 피크 -3.0 dBFS에
    **-22.1 LUFS** — 크레스트 19 dB다. 험이나 히스를 올리면 라우드니스가 붙지만 **1 dB당 액센트
    분리가 정확히 1 dB씩 죽는다** (실측: 험 0.20에서 분리 7.2 dB, 0.40에서 1.4 dB, 그 대가가 레벨
    3.9 dB). 하모닉스를 얹어 K대역으로 올리는 건 광대역 히스보다 낫지만 효과는 작다.
  - **AAC 인코딩이 인터샘플 피크를 약 1.8 dB 올린다.** WAV에서 -1.5 dBTP로 맞춰도 `-c:a aac`
    후에 **+1.7 dBTP**가 된다. 즉 `loudnorm`의 `TP=`는 최종 천장이 아니다. 인코딩 전 천장을
    **-4 dBFS 근처**로 두고 **먹싱된 파일을 다시 측정**한다.
  - **트랜지언트가 많은 베드는 컴프레서 없이 리미터만 쓰는 게 낫다.** 같은 -14.3 LUFS인데
    컴프레서를 빼자 헤드룸(-3.1→-3.7 dBTP)·다이내믹(LRA 2.9→3.5)·컷 액센트 분리(0.5~5.7→3.4~7.0 dB)가
    **동시에** 개선됐다. 천장이 정말 안 잡힐 때만 압축한다.
- **하드컷 키트에 종료 페이드를 넣지 않는다.** `MOTION.overlap=16`으로 실제로 겹치는 크로스페이드
  키트에서 가져오면, Sequence가 인접한 키트에서는 막을 게 없고 **컷당 6프레임씩 빈 프레임만 만든다**
  (v001에서 f65·f191·f677·f678, 25초 릴스에 1초). 또 **`at(f, 0, n)`은 f0에서 0이다** — 페이드 시작을
  0으로 옮겨도 첫 프레임은 여전히 비므로, 페이드를 없애야 한다.

### NEWSDESK — 뉴스 보도 포맷 (2026-09-04 대표님 제안·승인, 파일럿 1편 통과)

**키트 `remotion/NewsdeskKit.tsx`. 기획 근거는 `output/reels/NEWSDESK_FORMAT_BRIEF.md`.**
릴스 중앙 700회 대 카드뉴스 901회, 릴스 **평균 시청 중앙값 7초**, `corr(시청, 조회) = 0.62`.
이긴 릴스는 전부 판정·결정형 첫 문장, 진 릴스는 전부 논설조 선언이었다. 뉴스 문법은 본디 전자다.

- **컷마다 출처 등급을 단다: `FILE PHOTO` / `FILE` / `ARCHIVE` / `ILLUSTRATIVE`.**
  이게 일반 B롤을 정직하게 만드는 장치다. **면죄부가 아니다** —
  **문장의 구체성과 화면의 구체성을 일치시킨다.** 장소를 지목하면 그 장소의 사진을 쓴다.
- **정지컷은 허용된다. 금지 항목은 `excessive still-image ZOOMS`, 즉 정지컷 "줌"이다.**
  줌 없이 고정으로 물리고 로어서드·티커·FIGURE가 움직임을 공급하면 켄번스가 아니다.
- **모션카드 0장.** 키트에 개념이 없다.
- **레이아웃 전에 `remotion/tokens/core.ts`를 읽는다.** `SAFE.bottom=320`이라 **y1600 아래는
  인스타 캡션·CTA 영역**이고, `SAFE.actionRail`이 **y1100 아래 x930부터**를 버튼으로 덮는다.
  → **y1100 아래 사용 가능 상자는 x60~x900.** 첫 설계가 티커를 y1850에 두어 안 보일 뻔했다.
- **대본 예산은 실측 낭독에서 나온다.** Daniel(`anchor_deadpan` 레인) **2.01 단어/초**,
  26초 릴스 실사용 **35~45단어**. 비트 경계는 `silencedetect`로 뽑아 프레임에 고정한다.
- **톤: 앵커는 절대 농담하지 않는다.** 웃음은 사소한 사실을 정색하고 다루는 낙차에서 나온다.
  이모지·느낌표·밈 폰트 금지 — 정색이 무너지면 포맷이 죽는다.
- **주제 적합**: 날짜 있는 기관 발표 + 반전이 있는 글. 후보는 `192`·`232`.

### Reels 2.2 구조 기준 (2026-07-21, 3세대 실측 분석 기반 — 푸티지 기반 키트에 적용)
...
2. **2차 (조건부)**: K팝 굿즈·뷰티 제품. **확장 조건: 1차 레인 신규 글 코호트의 _페이지 중앙값_ CTR ≥ 1.5%.**
   (2026-08-20 정정: 원래 "집계 CTR"이었으나 그 지표가 잘못됐다 — 아래 "측정 규칙" 참조.)
...
### 제목 Korea 게이트 (2026-07-31 대표님 지시 — 신규·수정 공통 필수)

**모든 글 제목에 한국 지시어가 하나는 들어간다.** `Korea` / `Korean` / `Seoul` / `Busan` /
`K-pop` / `K-drama` / `Hangul` / `Jeju` 등. 리뷰어 스크립트가 하드 블로커로 검사한다.

- **왜**: 대표님 논지 — 이건 그 글 하나의 순위 문제가 아니라 **사이트 전체를 구글이 "한국 주제"로
  묶게 만드는 신호** 문제다. 노출이 큰 페이지일수록 신호 가치가 크다. 실제로 `090`은 혼자
  **158,280 노출**(사이트의 상당 부분)을 만들면서 제목에 Korea가 없었다 — 그 노출을 신호로 못 쓰고 있었다.
- **로마자 제품명만으로는 통과 못 한다.** `Chapagetti Guide`(X) → `Chapagetti: Korea's Black Bean Noodle`(O).
  `Deli Manjoo`(X) → `What Is Deli Manjoo? Korea's Subway Custard Snack`(O).
- **한국 지명만 있는 것도 부족하다.** `Suwon Day Trip Guide`(X) → `Suwon Day Trip from Seoul`(O).
  외국인은 수원·경주·대전을 한국 도시로 인식하지 못한다.
- **주의: 이건 필요조건이지 충분조건이 아니다.** Korea를 넣는다고 정의형 dead-end가 살아나지 않는다.
  `090`은 4~5위에서도 CTR 0.3%대였고 원인은 제목이 아니라 **쿼리 형태**였다.
  쿼리 형태 필터(행동/결정 프레임)를 통과한 다음에 이 게이트를 적용한다.
- 2026-07-31 기준 312편 전수 점검 완료 — 누락 21편을 일괄 수정해 **현재 100%**다.
...
- 리프레시 1편: 스펙 v1을 기존 글에 적용. **[2026-08-21 변경] 대상을 고르기 전에 `npm run seeds:check`를 먼저 돌린다.**
  - **왜 게이트가 앞에 붙었나 (2026-08-21 실측)**: 7/28~8/14 리프레시를 사전 클릭 유무로 갈랐더니,
    **클릭 0이던 옛 글 8편(`018 029 032 038 062 083 093 172`)을 고쳐서 8월 클릭이 0**이었다.
    같은 기간 클릭이 있던 17편은 71클릭을 냈다. **죽은 글은 고쳐도 안 살아난다.**
    `refresh-queue.json`은 층 안에서 노출로 정렬하는데(아래 참조, 그 자체는 타당하다),
    그러면 **노출 크고 클릭 0인 페이지가 계속 큐 앞으로 온다.** 필요한 건 정렬 변경이 아니라 **필터**다.
  - **판정선은 0과 1 사이다. 2가 아니다.** 초기 클릭별 이후 평균 클릭: **0 → 0.06** / 1 → 1.17 /
    2~3 → 1.75 / 4~9 → 2.21 / 10+ → 4.75. 0에서 1로 갈 때 **20배** 뛰고 그 뒤는 완만하다.
    **클릭이 하나라도 있으면 살아 있는 것**으로 본다. 근거: `output/strategy/breakout-prediction_2026-08-21.md`.
  - **발행 전 예측은 불가능하다는 것도 같이 쟀다.** 제목 길이 1.01배·제목 고유명사 1.02배·
    단어수 1.11배·카테고리 무차이. **"제목에 로마자 한국어 단어"** 가설은 평균 12.29 대 4.67로 보이지만
    **상위 3편(090·071·043)을 빼면 1.44 대 3.70으로 뒤집힌다.** 그 패턴은 그 세 편이 전부다.
    그래서 전략은 예측이 아니라 **많이 뿌리고 → 4~8주에 판정하고 → 산 것에만 붓는 것**이다.
  - **`seeds:check`가 지목한 dead-end는 존중한다.** 첫 실행에서 스크립트가 `301`·`210`을 추천했는데
    둘 다 CLAUDE.md가 이미 금지한 정의형 dead-end였다. 하드코딩 목록(`090 082 210 301`)에 더해
    **노출 1,000+ & CTR 0.5% 미만**이면 새로 생긴 dead-end로 자동 표시한다.
  - **끝난 시즌이 1위로 오르지 않게 최근 2주 클릭으로 줄 세운다.** `198` 워터밤은 창 안에서 76 → 8이었고,
    총합으로 세우면 사이트 최강 페이지로 1순위에 올랐다. 축제는 7월에 끝났다.
  - 작업을 마치면 `content/data/seed-check.json`의 해당 슬러그에 `"worked": "YYYY-MM-DD"`를 적는다.
    45일간 다시 지목되지 않고, `worked`·`note`는 재생성해도 보존된다.
- 리프레시 대상 자체는 여전히 **`content/data/refresh-queue.json`에서 위에서부터 꺼낸다 (2026-08-06 신설)** — 단 위 게이트를 통과한 것만.
  - **왜 파일로 만들었나**: 이 줄은 원래 "음식-구체 16편 먼저, 그다음 클릭 상위 30편"이라는 **순서만** 있고 목록이 없었다. 그래서 세션마다 즉석 스크립트로 다시 뽑았고, **필터에 따라 개수가 20편에서 56편까지 흔들렸다.** 게다가 그 "16편"은 애초에 글 목록이 아니라 **GSC 클릭 클러스터 개수**였다. 실제로 08-06의 `275`·`274`는 음식-구체가 아니라 클릭 상위군에서 꺼낸 것인데 순서를 건너뛴 걸 아무도 알아채지 못했다.
  - **재생성**: `npm run refresh:queue`. **층(tier)은 편집 판단이라 스크립트에 슬러그를 명시**하고, **결손(₩·한글·연도 스탬프)은 매번 파일에서 실측**한다. 그래서 글을 실제로 고치면 큐에서 저절로 빠지고, 체크를 깜빡해서 남는 일이 없다. `status`와 `note`는 재생성해도 보존된다.
  - **층 정의**: `1` 제품·브랜드 이름이 나오는 글(스펙이 그대로 적용) → `2` 장소·행동 글(가격·영업시간·출구번호로 착지) → `3` 음식이 나오지만 실은 문화 설명글(스펙이 잘 안 맞아 후순위).
  - **층 안에서는 클릭이 아니라 노출로 정렬한다.** 이 큐 52편의 **클릭 최대값이 7**이라 클릭으로는 순서를 못 매긴다. 노출은 "구글이 이미 몇 명에게 보여주고 있는가"이고 그게 리프레시가 실제로 작용하는 지점이다. **이건 "시계열 비교는 클릭으로만" 규칙과 충돌하지 않는다** — 그 규칙은 노출을 *시간에 걸쳐* 비교하지 말라는 것이고, 여기서는 한 추출본 안에서 페이지를 줄 세우는 것이다.
  - **필터 오탐은 스크립트의 `EXCLUDE`에 사유와 함께 남긴다.** 현재 4건: `261`·`265`는 `chip`이 반도체인데 과자로 잡혔고, `260`(밥솥)·`287`(그릴)은 가전이다. 조용히 거르지 말 것 — 다음 사람이 같은 오탐을 다시 발견하게 된다.
  - **[2026-08-20 신설] 큐에는 레인이 둘이다. 스펙 레인이 마르면 `push` 레인을 쓴다.**
    - **스펙 레인**(`queue`)은 "무엇이 빠졌나"를 묻는다. 2026-08-20에 **tier 1·2가 소진**됐고 26편 남은 게 전부 tier 3다. 스크립트가 소진 시 경고를 찍는다.
    - **푸시 레인**(`push`)은 다른 질문을 한다 — **"구글이 이미 어디서 우리를 4~9위로 많이 보여주고 있나."** 사이트 CTR이 1~3위 1.53% 대 5~11위 0.14%이고 노출의 87%가 5위 밖이라, 순위를 한 계단 올리는 게 실제 회수 지점이다.
    - **스펙 레인은 구조적으로 최고 기회를 못 봤다.** `!specApplied` 필터가 ₩·한글이 들어간 순간 글을 영구 제외했고 음식 필터가 비음식 글을 아예 안 들였다. 그래서 `071`(8,259노출이 4~9위)이 "완료"로 분류돼 안 보였다. 푸시 레인은 **스펙 완료 여부를 보지 않는다.**
    - **노출을 그냥 세지 않고 전환 가능한 형태만 센다.** 4~9위로 순위를 통제하고 dead-end 4편을 뺀 실측(2026-08-20): 행동형 **1.324%**(상위3 제거 후 1.089%) · 중립(제품·장소명) 0.664%(0.615%) · why형 0.156%(**0.000%**) · 정의형 0.060%(**0.000%**). **정의형·why형은 상위 3개를 빼면 클릭이 정확히 0** — 순위를 올려도 회수되지 않는다. `deadShare`가 그 비중이고, 높으면 노출이 커도 값어치가 없다.
    - **봇 쿼리를 먼저 버린다.** 따옴표 연산자 쿼리(`"hongdae station" "underground shopping mall"`)는 순위추적 도구이고 클릭하지 않아 "좋은 순위·0클릭"으로 위장한다. 이 필터가 없으면 레인의 1순위가 `074`가 된다 — 실제로 그럴 뻔했다.
    - `PUSH_EXCLUDE`에 사유와 함께 남긴다. 현재 6건: dead-end 4편 + `074`(봇 69%) + `181`(`naver webtoon`은 네이버로 가려는 내비게이셔널 쿼리라 1위여도 안 눌린다).
    - 작업한 페이지는 `PUSH_WORKED`에 판정일과 근거 파일을 적는다. **효과 판정은 CTR이 아니라 순위로 한다.**
- **리프레시는 스펙 점검 전에 "이 글이 아직 사실인가"를 먼저 본다 (2026-08-04 신설).** 날짜·행사·가격·
  장소가 들어간 글은 발행 시점에 맞던 것이 지금 틀렸을 수 있다. 실제 사례: `198`(워터밤)은 **이미 끝난
  행사를 미래형으로** 쓰고 있었고, 더 나쁘게는 확정 전에 쓰느라 **장소를 일부러 얼버무리고** 있었다
  ("if the venue is outside central Seoul"). 그 사이 축제는 **2024년에 잠실에서 고양 킨텍스로 이전**해
  있었다 — 즉 글 제목의 "Seoul"이 독자를 틀린 도시로 보내고 있었다. 스펙 v1의 다섯 항목은 이 검사를
  통과한 뒤에 적용한다. **얼버무린 문장은 리프레시에서 가장 값어치 있는 자리다** — 그때는 못 쓴 사실을
  지금은 쓸 수 있다.
- **리프레시는 이미지를 안 건드려도 캡션을 전수 재검한다 (2026-08-12 신설).** 두 가지가 걸린다.
  - **낡은 캡션이 애초에 사진과 안 맞는 경우.** `145` 실측: 캡션 4개 중 **3개가 사진을 반박**하고 있었다
    (세 그릇 오버헤드에 "매운 비빔이 쉽다" 설명 등). 자동 검사(`audit:image-context`)는 전부 통과했다 —
    **파일을 열어야만 잡힌다.** 게다가 나 자신이 같은 리프레시 초안에서 **평양식 물냉면 사진에
    "Bibim style"** 이라고 새 오캡션을 달았다. 2026-08-03 카드뉴스 짜파게티 사고와 같은 유형이고,
    하필 두 스타일 차이가 주제인 글이라 더 나빴다.
  - **본문이 정밀해지면 이전에 무해했던 사진이 반증 자료가 된다.** `145` 리프레시는 본문에
    **"메밀면은 회색에 반점, 밀면은 순백"** 이라는 식별 기준을 새로 넣었다. 그 문장이 페이지에 오르는
    순간, 면이 희고 반점 없는 기존 사진은 독자가 방금 배운 기준으로 **틀렸다고 판정할 자료**가 된다.
    **스펙 v1로 사실을 촘촘히 넣을수록 기존 사진과 충돌할 표면이 늘어난다** — 새 사실을 넣었으면
    그 사실로 기존 사진을 다시 본다.
...
- **집계 CTR로 코호트를 판정하지 않는다. 중앙값으로 한다 (2026-08-20 신설).**
  집계 CTR은 **전환되는 롱테일과 전환 안 되는 헤드 노출을 한 분모에 섞는다.** 실측: 사이트 전체에서
  1.5%를 넘는 구간은 **1~3위뿐**(1.53%)이고 5~11위는 0.14%다. 즉 "집계 CTR ≥ 1.5%"는 사실상
  "모든 쿼리에서 3위 안에 들어라"를 요구하는 도달 불가 기준이었다. **중앙값으로 보면 신규 코호트가
  오히려 최고다** — 신규 1.08%(중앙순위 7.8) / 6~7월 0.68%(9.2) / 5월 이전 0.43%(11.9).
- **집계 차이를 발견하면 반드시 ① 중앙값과 ② "상위 몇 편 제거" 검정을 같이 돌린다 (2026-08-20 신설).**
  이 저장소에서 같은 실수가 **두 번** 나왔다. 2026-07-31 "장소+결정 2.8배"(워터밤 한 편이 캐리)와
  2026-08-20 "제목 행동 단서 1.7배"(리프레시 3편이 캐리, 중앙값은 0.61 대 0.60으로 동일).
  **두 번째 건은 게이트를 만들기 직전에 잡았다 — 검정을 먼저 돌린 덕분이다.**
- **리프레시의 성과는 CTR이 아니라 클릭으로 판정한다 (2026-08-20 신설).** 리프레시분은 페이지당
  10.3클릭 대 3.2클릭(3.2배)인데 **중앙값 CTR은 0.45% 대 0.75%로 더 낮다.** `refresh-queue`가
  노출 높고 CTR 낮은 페이지를 골라 보내기 때문이다. 낮은 CTR을 실패로 오독하지 말 것.
...
1. **수요 근거**: GSC 쿼리, 검증된 Reels/카드뉴스 반응, 또는 외부 검색 근거 중 하나를 반드시 댄다.
   근거 없는 "좋아 보이는 주제" 금지.
2. **중복 감사**: **`node scripts/topic-dedup.mjs <로마자> <한글>`로 돌린다.** 제목·본문·
   `topics-queue.json`·`output/final`을 함께 훑고 **제목 적중은 하드 블록**이다.
   의미·검색의도가 겹치면 제목이 달라도 중복이다. 리트레드면 그렇게 라벨하고 기존 슬러그와 이유를 적는다.
   - **로마자 이름을 반드시 같이 넘긴다. 이게 실제로 잡아내는 축이다 (2026-09-03 신설).**
     글이 영어로 쓰이므로 **한글은 누군가 병기하기로 한 자리에만** 나온다. 한글로만 훑으면 통과한다.
     실패 2건: 8/26b가 맥심을 "전용 제목 0"으로 통과시켰으나 `278`과 `Maxim vs G7` **두 편**이
     있었고, 9/03에 붕어빵을 한글로 훑어 0건을 받았으나 `416`이 있었으며 **`071`이 이미 거기로
     링크를 걸고 있었다.**
   - **내부에 없다는 것은 신호가 아니다.** 커버리지 게이트(영어권이 이미 썼는가)를 **먼저** 돌린다.
...
### 영어권 음식 커버리지는 포화다 — 신규가 안 나오면 리프레시로 대체한다 (2026-09-03)

**세 사이클 연속 통과 0건이다.** 8/26(부침가루·미숫가루·홍삼·햇반) 0건 → 8/26b 1건이나 오답
(이미 발행된 글) → 9/03(십원빵·탕후루·꿀타래·계란빵·닭강정) 0건.
**영어 수요가 생길 만큼 유명한 한국 식품은 이미 ①영어 위키피디아 전용 문서 ②Maangchi 계열
레시피 블로그 ③Korea Herald/Times 영문 중 최소 둘을 갖고 있다.**

- **"안 쓰인 사실 층"을 노리는 8/26 방법도 이 포화를 못 뚫는다.** 십원빵은 한국은행
  2024-09-01 도안 이용기준 개정이라는 결말이 영어에 없을 것 같았는데 **영어 위키피디아에
  들어 있었다.** 층이 비어 보이면 **영어 위키피디아부터 열어본다.**
- **죽은 층과 산 층의 차이는 주제가 아니라 "거래 정보"인지다.** *"이 음식이 무엇인가"*는 영어가
  갖고 있고, **얼마인가 · 몇 번 출구인가 · 언제 물건이 들어오는가 · 언제 붐비는가**는 없다.
  `201`·`249`·`145`·`242`가 전부 그 형태로 성공했다.
- **따라서 게이트를 통과하는 신규가 없으면 그 자리를 리프레시로 채운다.** 약한 신규를 억지로
  발행하지 않는다. 데이터도 그렇게 말한다 — **리프레시 페이지당 10.3클릭 대 신규 3.2클릭**(2026-08-20).
...
2. **이미지는 주제 확정 후, 다음 단계로 조달한다 (막히면 다음 단계로, 절대 주제를 버리지 않는다):**
   - **0차 (2026-08-03 신설): 대상이 포장 제품·브랜드 상품·기업이면 제조사/기업 공식 사이트를 먼저 본다.** 아래 1~4차는 전부 스톡·공공 아카이브·문화유산 소스라 **특정 상품의 팩샷이라는 범주 자체가 없다.** 이 단계를 건너뛰면 Commons에서 "비슷한 라면 사진"을 찾아놓고 조달에 성공했다고 착각한다 — 2026-08-03 라면 카드뉴스가 정확히 그렇게 실패했다. 저장소에 이미 선례가 있었다: `buldak-original-product.jpg`(samyangfoods.com 공식, 2026-07-15 커밋)를 카드뉴스(2026-08-02)보다 18일 먼저 쓰고 있었는데 일반화하지 않았다.
     ...
     - **단, 피사체 자체가 흰색이면 이 기법은 쓸 수 없다 (2026-08-17 정정).** 위 조건을 스킨케어 4병에
       그대로 적용했더니 **3병이 통째로 지워지고 검은 뚜껑과 인쇄 라벨만 남았다.** 크림색 차체는 채도
       44지만 **젖빙 유리병·투명병은 채도 ~2에 휘도 ~235** 라 배경 판정과 수치상 구별이 안 되고,
       테두리가 부드러워 플러드필의 연결성이 끊기지 않아 병 내부까지 걸어 들어간다. **"밝은 무채색
       스윕 위의 제품"은 한 가지 경우가 아니다** — 피사체 채도를 먼저 재고, 10 미만이면 컷아웃을
       포기한다. 대안은 **원본 스윕을 그대로 살려 제품 패널로 쓰는 것**이고, 이게 더 정직하다(스튜디오
       제품컷은 실제로 진열 태그처럼 읽힌다). 비율만 통일하면 레이아웃 슬롯이 고정된다.
       실측: `output/reels/2026-08-17_kbeauty-picker/prep-cutouts.mjs` 헤더.
...
> **경고 (2026-08-12 사고): 이 스텝은 읽기 전용이 아니다. `--slug`는 "이 주제를 조사하라"는 뜻이 아니다.**
> **큐에서 제 주제를 골라 `content/data/topics-queue.json`을 수정한다** — 승인된 큐 밖 주제(`378`
> 불꽃축제)로 돌렸더니 갑질(id 147)을 골라 `status: in_progress` + `generated_slug: "378"`로 바꿔놨다.
> 즉 **큐 순번에 그 슬러그를 붙이는 동작**이라, 큐에 없는 주제에는 쓸 수 없다.
> `git checkout content/data/topics-queue.json`으로 원복했다.
> **대표님 승인을 받은 큐 밖 주제(주간 키워드 사이클 통과분 등)는 이 스텝을 건너뛰고 직접 조사·집필한다.**
> 위의 `--step review` 경고와 같은 계열이다 — **파이프라인 스텝은 부작용이 있다고 가정하고,
> 돌린 뒤 `git status`로 무엇이 바뀌었는지 확인한다.**
...
### 카드뉴스 예약은 스크립트로 한다 — 그리고 캡션을 반드시 되읽는다 (2026-09-03 신설)

도구 3종이 저장소에 있다. **`.tmp/`에 다시 만들지 말 것** — gitignore라 세션마다 사라져서
판독기만 세 번(v1·v2·CDP판) 새로 쓰였다.

```bash
python scripts/launch-meta-chrome.py                      # 먼저 띄워두고 (CDP :9222)
python .claude/skills/cardnews/scripts/schedule-meta-cardnews.py public/assets/cardnews/{폴더} {일} 오전 5 00 [--commit]
#   --commit 없으면 예약 버튼 직전에서 정지하고 스크린샷만 남긴다
python .claude/skills/cardnews/scripts/read-meta-scheduled.py     # 예약 달력 재판독
python .claude/skills/cardnews/scripts/fix-meta-caption.py "{행 제목 접두사}" public/assets/cardnews/{폴더} [--commit]
#   예약된 글의 캡션만 재업로드 없이 수리
```

- **런처가 떠 있으면 나머지는 CDP로 붙는다.** 같은 프로필에 persistent context를 또 열면
  프로필이 잠겨 있어 실패하거나 **로그인 없는 새 프로필**로 뜬다.
- **작성기가 타이핑 중에 캡션을 조용히 회전시킨다.** `#` 해시태그 헬퍼와 URL 자동링크가
  캐럿을 offset 0으로 되돌려, 그 뒤 입력이 글 맨 앞에 꽂힌다. 2026-09-03에 `438`(해시태그 줄이 앞으로)과
  `223`(26번째 줄 `k-eta.g|o.kr`에서 갈림) **2편이 그 상태로 예약됐다.**
  - 처방은 두 겹이다: **`page.keyboard.type()` 대신 `insert_text()`로 한 줄씩** 넣고(입력 이벤트 1회라
    헬퍼가 안 깨어난다), **입력 후 편집기를 되읽어 원본과 공백 제거 비교**한다. 3회까지 재시도.
  - **눈으로는 못 잡는다.** 같은 시각의 `194`는 멀쩡했고(타이밍 의존), **사전 스크린샷은 편집기의
    가운데만 보여준다.** 실제로 수리 6회 중 4회가 1차 실패·2차 통과였다 — 1회 입력은 신뢰 불가.
- **예약된 글의 캡션은 재업로드 없이 고칠 수 있다**: 행 `...` → `게시물 관리`(**hover**) → `게시물 수정`.
  **하루치는 두 행(FB·IG)이고 한 번에 한 행만 고쳐진다.** 그리고 **행은 접두사로 찾아야 한다** —
  고쳐진 캡션도 끝에 같은 해시태그가 있어서 부분일치는 **이미 고친 행을 다시 연다.**
- **시·분은 폼에서 되읽을 수 없다** (spinbutton의 `aria-valuetext`·`.value` 둘 다 빈 문자열, AM/PM만 노출).
  시각 확인은 **사전 스크린샷**(`오전 05:00`)과 **커밋 다이얼로그**(`2026. 9. 20. 오전 5:00`)로 한다 —
  예약 목록 텍스트에는 오전/오후가 안 찍힌다.
- **캡션 파일은 `caption.txt`다.** 옛 이름 `instagram-caption.md`를 하드코딩하고 있던 탓에 8/16 이후
  처음 돌린 날 첫 폴더에서 죽었다.

### 릴스 예약도 스크립트로 한다 — 컴포저가 달라서 규칙이 갈린다 (2026-09-04 신설)

```bash
# 릴스는 attach가 아니라 launch다. 런처 Chrome이 떠 있으면 먼저 닫는다.
python .claude/skills/reels/scripts/schedule-meta-reel.py {mp4} {caption.txt} {일} 오전 5 00
python .claude/skills/cardnews/scripts/read-meta-scheduled.py --own-browser --slices
```

- **릴스 컴포저는 카드뉴스 작성기와 다른 화면이고, 되읽을 수 있는 것도 다르다.** 위 카드뉴스
  항목의 *"시·분은 폼에서 되읽을 수 없다"* 는 **카드뉴스 작성기 한정**이다. 릴스 컴포저의
  spinbutton은 `aria-valuetext`로 상태를 내주므로 **시·분·오전오후를 실제로 검증하고 커밋한다**
  (`input_value()`는 여기서도 항상 빈 문자열이다 — 그걸로 비교하는 루프는 종료 조건이 없어서
  ArrowUp을 최대 횟수까지 누르고 아무 데나 착지한다. 실제로 카드뉴스 배치 한 건이 그렇게 오후로
  예약된 적이 있다).
- **`connect_over_cdp`로 붙이면 안 된다.** Playwright는 자기가 띄우지 않은 브라우저에 50MB 초과
  업로드를 거부한다. 그래서 이 스크립트만 persistent context를 직접 띄우고, **런처가 같은 프로필을
  잡고 있으면 실패하거나 로그인 없는 새 프로필로 뜬다.** 릴스 예약 전에는 Chrome을 닫는다.
- **캡션 회전 게이트를 릴스 스크립트에도 이식했다 (2026-09-04).** 원래 `keyboard.type(line, delay=3)`
  이라 2026-09-03에 카드뉴스 5편 중 2편을 망가뜨린 것과 **정확히 같은 코드**였다. 이제 카드뉴스와
  같이 `insert_text()` + 편집기 되읽기 + 3회 재시도이고, 안 맞으면 **아무것도 예약하지 않고 종료**한다.
  4편 모두 1차 통과했다.
- **날짜가 찼다는 것과 맞는 글이 거기 있다는 것은 다르다.** `--slices`가 행마다 캡션 첫 글자를
  찍어주므로 그걸로 대조한다 (2026-08-20에 인덱스는 musinsa를 09-17로, 플래너는 09-03 오후 8:00에
  다른 글 위에 겹쳐 갖고 있었다). **부수 효과로 캡션 회전 여부도 같이 확인된다** — 회전한 글은
  첫 글자가 해시태그다.
- **릴스 인코딩 하한선 8 Mbps는 푸티지 키트 기준이다.** DOSSIER 같은 평면 디자인 키트는
  1.5~1.8 Mbps가 정상이고, 판정은 전체 해상도에서 글자 경계를 보는 것으로 한다. 이 4편 배치가
  10.0 / 1.6 / 1.5 / 1.8 Mbps로 섞여 있는 것은 그래서다.
